...
function  AbPinChild() {
    ...
    const [symptomData6, setSymptomData6] = useState([]);
    const [selectedPains, setSelectedPains] = useState([]);
    const [selectedOnset, setSelectedOnset] = useState([]);
    const [selectedPreceded, setSelectedPreceded] = useState([]);
    ...
    useEffect(() => {
        const fetchSymptoms = async () => {
            ...
            try {
              const result1 = await session.run('MATCH (n {SymptomID:27 })-[r:Pain_is]-(m) RETURN m.PainID, m.PainName');
              const options = result1.records.map(record => ({
                value: record.get('m.PainID'),
                label: record.get('m.PainName')
              }));

              const result2 = await session.run('MATCH (n {SymptomID:27 })-[r:Onset_is]-(m) RETURN m.OnsetID, m.OnsetName');
              const options2 = result2.records.map(record => ({
                value: record.get('m.OnsetID'),
                label: record.get('m.OnsetName')
              }));

              const result3 = await session.run('MATCH (n {SymptomID:27 })-[r:Triggered_or_worsened_by]-(m) RETURN m.TriggeredID, m.TriggeredName');
              const options3 =result3.records.map(record => ({
                value: record.get('m.TriggeredID'),
                label: record.get('m.TriggeredName')
              }));
              
              const result4 = await session.run('MATCH (n {SymptomID:27 })-[r:Relieved_by]-(m) RETURN m.RelievedID, m.RelievedName');
                const options4 =result4.records.map(record => ({
                    value: record.get('m.RelievedID'),
                    label: record.get('m.RelievedName')
                }));
                const result5 = await session.run('MATCH (n {SymptomID:27 })-[r:Accompanied_by]-(m) RETURN m.AccompaniedID, m.AccompaniedName');
                const options5 =result5.records.map(record => ({
                    value: record.get('m.AccompaniedID'),
                    label: record.get('m.AccompaniedName')
                }));
                const result6 = await session.run('MATCH (n {SymptomID:27 })-[r:Preceded_by]-(m) RETURN m.PrecededID, m.PrecededName');
                const options6 =result6.records.map(record => ({
                    value: record.get('m.PrecededID'),
                    label: record.get('m.PrecededName')
                }));
...
const handleOnset = (event) => {
    if (event.target.checked) {
        setSelectedOnset([...selectedOnset, event.target.value]);
        console.log(selectedOnset);
    } else {
        setSelectedOnset(selectedOnset.filter(pain => pain !== event.target.value));
        console.log(selectedOnset);
    }
};

const handlePreceded = (event) => {
    if (event.target.checked) {
        setSelectedPreceded([...selectedPreceded, event.target.value]);
        console.log(selectedPreceded);
    } else {
        setSelectedPreceded(selectedPreceded.filter(pain => pain !== event.target.value));
        console.log(selectedPreceded);
    }
};
...
const handleSubmit = async (e) => {
    e.preventDefault();
    const session = driver.session();
    const dataSymptomp = { patient_ID , symptom_ID, pain_ID: selectedPains, Onset_ID: selectedOnset, preceded_ID: selectedPreceded, triggered_ID: selectedTriggered, relieved_ID: selectedRelieved, accompanied_ID: selectedAccompanied};
            ...
            <div style={{ display: 'flex', flexDirection:'row' }}>
                <div className="form first" style={{ display: 'flex', flexDirection: 'column', maxWidth: '100%'}}>
                                ...
                                {symptomData2.map((item, index) => (
                                    ...
                                    <div key={index} style={{ marginLeft: '10px', marginRight: '15px', marginBottom: '10px' }}>
                                        <input 
                                            type="checkbox" 
                                            id={`Onset-${item.value}`} 
                                            name={`Onset-${item.value}`} 
                                            value={item.value} 
                                            onChange={handleOnset}
                                        />
                                        <label htmlFor={`Onset-${item.value}`}>{item.label}</label>
                                    </div>
                                ))}
                         </div>
                    </div>
                    <div className="details personal">
                        <h2>Preceded by</h2>
                        <div className="select">
                                {symptomData6.map((item, index) => (
                              
                                    <div key={index} style={{ marginLeft: '10px', marginRight: '15px', marginBottom: '10px' }}>
                                        <input 
                                            type="checkbox" 
                                            id={`Preceded-${item.value}`} 
                                            name={`Preceded-${item.value}`} 
                                            value={item.value} 
                                            onChange={handlePreceded}
                                        />
                                        <label htmlFor={`Preceded-${item.value}`}>{item.label}</label>
                                    </div>
                                ))}
                         </div>
                    </div>
