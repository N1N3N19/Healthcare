...
function  ConstipationinAdult() {
    ...
    useEffect(() => {
        const fetchSymptoms = async () => {
            ...
            try {
              const result1 = await session.run('MATCH (n {SymptomID:44 })-[r:Blood_appears]-(m) RETURN m.AppearID, m.BloodAppearsName');
              const options = result1.records.map(record => ({
                value: record.get('m.AppearID'),
                label: record.get('m.BloodAppearsName')
              }));
              const result4 = await session.run('MATCH (n {SymptomID:44 })-[r:Problem_is]-(m) RETURN m.ProblemID, m.ProblemName');
                const options4 =result4.records.map(record => ({
                    value: record.get('m.ProblemID'),
                    label: record.get('m.ProblemName')
                }));
                const result5 = await session.run('MATCH (n {SymptomID:44 })-[r:Accompanied_by]-(m) RETURN m.AccompaniedID, m.AccompaniedName');
...
const handleSubmit = async (e) => {
    e.preventDefault();
    const session = driver.session();
    const dataSymptomp = { patient_ID , symptom_ID, BloodAppears_ID: selectedBloodAppears, Problem_ID: selectedProblem, accompanied_ID: selectedAccompanied};
...
export default ConstipationinAdult;